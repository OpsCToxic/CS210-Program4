...
public:
  ...
  * @see        
  */
  static vector<CityNode> citiesSortedByNumOf_Its_ReachableCities_byTrain(vector<string> cities, vector<pair<string, string>> trainRoutes);
 
  static unordered_map<string, CityNode> makeCityGraph(vector<string> cities, vector<pair<string, string>> trainRoutes);

  static void depthFirstSearchReachableCities(unordered_map<string, CityNode> cityGraph, string startingCity, vector<string> &reachableCities, set<string> &visitedCities);
